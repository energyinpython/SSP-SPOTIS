...
class SSP_SPOTIS(MCDA_method):
    def __init__(self):
        """Create SSP_SPOTIS method object.
        """
        pass
    ...
    def __call__(self, matrix, weights, types, s_coeff = 0):
        ...
        >>> pref = ssp_spotis(matrix, weights, types)
        >>> rank = rank_preferences(pref, reverse = False)
        """
        SSP_SPOTIS._verify_input_data(matrix, weights, types)
        return SSP_SPOTIS._ssp_spotis(self, matrix, weights, types, s_coeff)
    ...
    @staticmethod
    def _ssp_spotis(self, matrix, weights, types, s_coeff):
        ...
        # Determine Ideal Solution Point (ISP) based on maximum and minimum bounds with reduced compensation based on matrix with reduced compensation
